...
# factored energy
net = readTopology("real_networks/sikora_2019.phy")
niter = 50 
cg = PGBP.clustergraph!(net, PGBP.JoinGraphStructuring(k))
sch = PGBP.spanningtrees_clusterlist(cg, net.nodes_changed)
df = select(CSV.read("data/sikora_univariatedata", DataFrame), [1,i+1])
...
# residual
residtrue = lltrue - fetrue # residual
# save results
CSV.write("data/pick_k/sikora_uv/k$(k)/rep$(i).csv",
